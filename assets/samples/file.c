 ...
 * events or callback on changes and no way to lock files.
 *
 ...
 * 2)  Do another "stat" and compare mtime again. Unfortunately the vboxsf host
 * API does not allow stat on handles, so we would need to use
 ...
 *     a) "stat" and compare mtime, on stat failure invalidate the cache
 *     b) "stat" and compare mtime, on stat failure do nothing
